...
if [ ! -f "$1/appenv" ]; then
  echo "You must enter a valid project for deployment"
  exit
fi
...
if [ ! -f ./target/upx ]; then
  ...
  tar xf upx-3.95-amd64_linux.tar.xz && cp ./upx-3.95-amd64_linux/upx target/upx && chmod +x target/upx && rm -rf upx-3.95-amd64_linux*
fi
