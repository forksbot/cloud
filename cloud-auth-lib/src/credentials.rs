...
impl Credentials {
    pub fn load_and_check<L, T>(credentials_file: &str, jwks_files: &[&str], scope: Option<L>) -> Result<(Credentials, String, TokenValidationResult), CloudAuthError>
    ...
    pub fn load_and_check_for_user<L, T>(credentials_file: &str, jwks_files: &[&str], scope: Option<L>, user_id: String) -> Result<(Credentials, String, TokenValidationResult), CloudAuthError>
        where L: IntoIterator<Item=T>, T: AsRef<str> {
        let mut credentials: Credentials = serde_json::from_str(credentials_file)?;
        for jwks_file in jwks_files {
            credentials.add_jwks_public_keys(serde_json::from_str(jwks_file)?);
    ...
    pub fn compute_secret(&mut self) -> Result<(), CloudAuthError> {
        use ring::signature;
        use biscuit::jws::Secret;

        let vec = pem_to_der(&self.private_key)?;
        let key_pair = signature::RsaKeyPair::from_pkcs8(&vec).map_err(|f| CloudAuthError::Generic(f.description_()))?;
    ...
    #[allow(dead_code)]
    pub fn add_pub_key(&mut self, key_id: String, pub_der: Vec<u8>) -> Result<(), CloudAuthError> {
        use biscuit::jws::Secret;
